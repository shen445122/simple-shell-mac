...
def checkStrategy(code_list, engine_string):
    ...
    for info in code_list: 
        if info.startswith('#') or info == '':
           continue       
        code = info.split()[0]
        price_tag = float(info.split()[1])
        stInfo = basefunc.getStockInfo(date_last, code, engine_string)
        price_low = stInfo['low']
        name = stInfo['name']
        if price_low < price_tag:
            print("[%s] %s 目标值：%.2f 当前值：%.2f" %(code, name, price_tag, price_low))
...
def checkRollbackBest(codes, number_of_assets, engine_string):
    ...
    for i in range(number_of_assets):
        ...
        if rate > 0:
            ...
            checkdata['code'] = returninfo
            checkdata['sharpe'] = {'yield':stoptsr3[0],'volatility':stoptsr3[1],'sharpe':stoptsr3[2]}
...
# 最优风险资产组合验收
def checkRollbackBestAcception(checkdatafile, checkdatatable, engine_string):
    from func import basefunc
    import datetime

    if not checkdatatable:
        year = datetime.datetime.now().strftime('%Y')
        month = datetime.datetime.now().strftime('%m')
        day = datetime.datetime.now().strftime('%d')
        checkdatatable = 'stock_' + year + '_' + month + '_' + day

    RBData = basefunc.changeFile2List(checkdatafile)
    Result = {}
    for RBDataSub in RBData:
        RBDataSub = eval(RBDataSub)
        RByield = RBDataSub['sharpe']['yield']
        RBvolatility = RBDataSub['sharpe']['volatility']
        RBsharpe = RBDataSub['sharpe']['sharpe']
        RBcode = RBDataSub['code']
        #输出json
        #if RByield < 0.9:
        #    continue
        ReturnCash = basefunc.getrReturnFromDict(RBcode, checkdatatable, 100000, engine_string)
        Result['收益率'] = RByield
        Result['波动率'] = RBvolatility
        Result['夏普比率'] = RBsharpe
        Result['现金收益'] = ReturnCash
        print(Result)
