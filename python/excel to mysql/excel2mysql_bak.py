...
def importDataHelper(username, password, database, host, port, datapath):
    try:
        conn = mysql.connector.connect(user=username, password=password, database=database, host=host, port=port, use_unicode=True)
        #conn = mysql.connector.connect(user="root", password="", database="stock", host="127.0.0.1", port=3306, use_unicode=True)
...
if __name__ == "__main__":
    importDataHelper(username, password, database, host, port, datapath)
